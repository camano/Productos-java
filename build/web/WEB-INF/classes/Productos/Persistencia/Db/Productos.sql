...
alter table usuario
add column usuarioEstado boolean;
...
DELIMITER ;
call ActualizarUsuario("j",123,"jcaamano8",1);

select * from consultarusuario where usuarioId=1;
delete from usuario where usuarioId=2;

select * from usuario inner join rol on usuarioRol=rolId;
update usuario set usuarioEstado=0;

alter table usuario
modify usuarioTelefono varchar(10);
describe usuario;

alter table usuario
modify usuarioEstado enum('Activo','Inactivo');
update usuario set usuarioEstado='Activo';
